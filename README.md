# 청포도 프로젝트 프론트엔드 팀 개발 규칙(초안)

청포도 프로젝트 프론트엔드 팀의 코드 작성 및 폴더구조 규칙을 정리한 문서입니다.<br/>
강요는 아니지만 코드를 리팩토링 및 이슈 해결시 혼돈이 안생기도록 규칙이 지켜주셨으면 감사하곘습니다.<br/>
PR 과정에서 규칙 위반 시 **리뷰 반려 또는 수정 요청**이 발생할 수 있습니다.<br/>
...
- **develop**
    - 모든 기능 개발의 기준이 되는 브랜치
    - 작업 시작 시 develop 기준으로 브랜치 생성
    - 직접 푸시 금지 → PR을 통해 merge

---

## 📌 기능 브랜치 (Feature Branch)

모든 UI, 로직, API 연동 등 **기능 개발은 feature 브랜치에서 진행**합니다.
...
```
fix/login-token-save
fix/post-list-pagination
fix/header-ui-overlap
```

---

## 📌 리팩토링 브랜치 (Refactor Branch)

기능 변화 없이 구조 개선, UI 정리, 폴더 재정리 등.
...
refactor/post-item-ui
```

---

## 📌 긴급 수정 브랜치 (Hotfix Branch)

main에서 발생한 **긴급 문제 해결용** 브랜치.
...
```
git add .
git commit -m "feat: 로그인 UI 구조 작성"
git push origin feature/login-ui
```

### 3. GitHub PR 생성 → 리뷰 요청 → 승인 후 merge

### 4. merge 후 feature 브랜치는 삭제해도 OK
...
- **브랜치 하나당 하나의 목적만**
- 브랜치명을 보면 어떤 작업인지 바로 이해 가능해야 함
- develop 최신 버전을 수시로 pull하여 충돌 최소화
- PR은 작을수록 좋음(리뷰 효율 증가)
- UI / 로직 / 설정이 섞인 브랜치는 지양
